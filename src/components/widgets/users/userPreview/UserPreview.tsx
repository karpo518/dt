...
export const UserPreview:FC<TProps> = ({username, firstName, lastName, roles, workBorders}) => {
  ...
  const wBordersList = workBordersVariants.filter(w => workBorders.includes(w.id)).map(w => w.name).join(', ')
  ...
  return (
    <Card className={s.wrapper}>
      <Card.Body>
        ...
        <Link className={cn('btn','btn-light', s.updateButton)} to={`/users/update/${username}`}>
          Редактировать
        </Link>
      </Card.Body>
    </Card>
